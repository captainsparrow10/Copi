 ...
 * through Ollama's OpenAI-compatible endpoint; `LLM_PROVIDER=google` uses
 * `@ai-sdk/google`. No other file should import `createOpenAICompatible` or
 * `google` directly for chat.
 */
...
export function getChatModel(): LanguageModel {
  return LLM_PROVIDER === "google" ? googleModel() : ollamaModel();
}

/** Which provider is configured (used by /api/health for a cheap reachability check). */
export function chatProviderName(): "ollama" | "google" {
  return LLM_PROVIDER === "google" ? "google" : "ollama";
}
